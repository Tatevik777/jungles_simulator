...
public class EventProducer {
    ...
    private void eatEvent(Elephant elephant) {
        ...
        energy = energy - 30;
        energyReset(elephant);
        health = health + (int) (elephant.ENERGY_RATIO_AFTER_EATING * 4);
        if (health > 500) {
            health = 500;
        }
        elephant.setEnergy(energy);
    ...
    private boolean statusChecker(Elephant elephant) {
        if (elephant.getHealth() <= 0) {return false;} else {return true;}}
    ...
    public void energyReset(Elephant elephant) {
        int energy = elephant.getEnergy();
        if (energy < 0) {
            energy = 0;
        }
